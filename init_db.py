...
def init_db():
    ...
    # --- SAFE FIX FOR THE SCHEMAS DISCONNECT ---
    # Drops the old schema structure so SQLite is forced to build the updated layout below.
    cur.execute("DROP TABLE IF EXISTS expense_logs")

    # 4. Expense logs table 
    cur.execute("""
    ...
    food_items_data = [
        ...
        ("Orange Juice", "Deen Cafe", 4.00, "Beverage", 120),
        ("Apple Juice", "Deen Cafe", 4.00, "Beverage", 110),
        ("Watermelon Juice", "Deen Cafe", 4.00, "Beverage", 90),
        ("Extra Joss", "Deen Cafe", 4.00, "Beverage", 30),
        ("Carrot Juice", "Deen Cafe", 3.50, "Beverage", 90)
    ]

    # Insert data safely using REPLACE to update prices/calories dynamically
    cur.executemany("""
    ...
    conn.commit()
    conn.close()
    print("Database initialized, updated, and synced successfully.")
